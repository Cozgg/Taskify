# Báo cáo Tuần 6
**Tuần:** 6 (23/03/2026 - 29/03/2026)

**Nhóm:** 1

**Đề tài:** E1 - Hệ thống quản lý công việc (Task Management - Trello Clone)

**Nhóm trưởng:** Phạm Anh Quyền - 2351010180

---
## 1. Công việc đã hoàn thành

| Thành viên | MSSV | Công việc | Link Commit/PR |
|------------|------|-----------|----------------|
| Phạm Anh Quyền | 2351010180 | - Thiết kế Database (MySQL) & Setup Server| [f0bd47deeab3c6bb8c85ce96bfd06f980669ad49](https://github.com/phamquyendev03/Taskify/commit/f0bd47deeab3c6bb8c85ce96bfd06f980669ad49) |
||| - API Xác thực (Register, Login (JWT), Profile) | [9cba50f1bed52b38b59dff0c03ba84c1f3005ac1](https://github.com/phamquyendev03/Taskify/commit/9cba50f1bed52b38b59dff0c03ba84c1f3005ac1) |
||| - API Quản lý (Workspace & Project) | [7720c33c8d3caa57f7bf4b68ee0eadc181a5d70e](https://github.com/phamquyendev03/Taskify/commit/7720c33c8d3caa57f7bf4b68ee0eadc181a5d70e) |
||| - API admin | [1299c53df73daf84987108a3c2226e3bc52c8f61](https://github.com/phamquyendev03/Taskify/commit/f5a4d55a8bc0985b275990ef46fb9968572cc88b) |
| Nguyễn Hữu Công | 2351010025 | - Thiết kế Database (MySQL) | [d72508b4ccb96331bcb4f14ca11fdfc7c7dc691d](https://github.com/phamquyendev03/Taskify/blob/d72508b4ccb96331bcb4f14ca11fdfc7c7dc691d/docs/architecture/Database.md) |
||| - API Cấu trúc (Board & Column) | [2b14972c55ab4b6c2a1a8900db326e437f6bd113](https://github.com/phamquyendev03/Taskify/blob/2b14972c55ab4b6c2a1a8900db326e437f6bd113/backend/Taskify/src/main/java/com/ccq/controller/ListController.java) |
||| - API Task (CRUD công việc) | [2b14972c55ab4b6c2a1a8900db326e437f6bd113](https://github.com/phamquyendev03/Taskify/blob/2b14972c55ab4b6c2a1a8900db326e437f6bd113/backend/Taskify/src/main/java/com/ccq/controller/CardController.java) |
||| - Logic cốt lõi (Xử lý kéo thả/di chuyển Task) |[7f3c83e521926f55d43b7cafe45ec0d3ebe0e50f](https://github.com/phamquyendev03/Taskify/commit/7f3c83e521926f55d43b7cafe45ec0d3ebe0e50f)|
| Huỳnh Thế Cảnh | 2351010022 | - API Thành viên (Mời user, Gán người làm task) | [4648072f4b51380ff57178c26253f89c57519c00](https://github.com/phamquyendev03/Taskify/commit/4648072f4b51380ff57178c26253f89c57519c00) |
||| - API Tương tác (Comment, Nhãn màu (Labels)) | [a9bf2b2097ad4958858ba64042d6eb26b315721b](http://github.com/phamquyendev03/Taskify/commit/a9bf2b2097ad4958858ba64042d6eb26b315721b) |
||| - API Thống kê (Báo cáo tiến độ & Trạng thái dự án) | [a9bf2b2097ad4958858ba64042d6eb26b315721b](http://github.com/phamquyendev03/Taskify/commit/a9bf2b2097ad4958858ba64042d6eb26b315721b) |
---
## 2. Tiến độ tổng thể
| Hạng mục | Trạng thái | % |
|----------|------------|---|
| Phân tích yêu cầu | Done | 100% |
| Thiết kế kiến trúc | Done | 100% |
| Backend API | Done | 100% |
| Frontend UI | Đang làm | 30% |
| Docker | Chưa bắt đầu | 0% |
| Testing | Chưa bắt đầu | 0% |
**Tổng tiến độ: 55%**
---

## 3. Kế hoạch tuần tới
| Thành viên | Công việc dự kiến |
|------------|-------------------|
| Phạm Anh Quyền | Hiện thực giao diện Auth, Workspace và Docker/CICD |
| Nguyễn Hữu Công  | Hiện thực giao diện Board, Card, Drag & Drop và Docker/CICD |
| Huỳnh Thế Cảnh | Hiện thực giao diện Comment, Member, Charts và Docker/CICD |

---
## 4. Khó khăn / Cần hỗ trợ

---
*Ngày nộp: 29/03/2026*

*Xác nhận của Nhóm trưởng: Phạm Anh Quyền*
